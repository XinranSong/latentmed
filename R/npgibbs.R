...
update_forest_Y <- function(theta_Y, XQY, forest_Y) {
  mu_theta_Y <- forest_Y$do_gibbs(XQY, theta_Y, XQY, 1)
  return(list(mu_theta_Y = mu_theta_Y, tau2_Y = forest_Y$get_sigma()^2))
}
...
update_theta <- function(mu_theta, Y, a, b, sigma2, tau2) {
  # Ensure inputs are matrices/vectors of correct orientation
  N <- nrow(Y)
  J <- ncol(Y)

  # Vectorized computation of posterior parameters
  Y_centered <- Y - tcrossprod(rep(1, N), a)  # Y - a[j]
  weighted_residuals <- Y_centered * tcrossprod(rep(1, N), b/sigma2)
  data_term <- rowSums(weighted_residuals)

  # Combine data term with prior contribution
...
update_theta_M <- function(mu_theta_M, theta_M, M, a, b, sigma2_M, tau2_M, theta_Y, forest_Y, XQ) {
  N <- length(theta_M)
  XQY <- cbind(XQ, pnorm(theta_M))
  prior_loglik <- dnorm(x = theta_Y,
                        mean = forest_Y$do_predict(XQY),
                        sd = forest_Y$get_sigma(),
                        log = TRUE)

  theta_M_prop <- update_theta(mu_theta_M, M, a, b, sigma2_M, tau2_M)
  post_loglik <- dnorm(x = theta_Y,
                       mean = forest_Y$do_predict(cbind(XQ, pnorm(theta_M_prop))),
                       sd = forest_Y$get_sigma(),
                       log = TRUE)
  U <- log(runif(N))
...
initialize_vars <- function(X, Y, M, n_burn = 2500, n_save = 2500) {
  ...
  ## Initialize theta_Y and theta_M
  ...
  fit_theta_Y <- gibbs_factor_analysis(Y = Y, X = cbind(X1, theta_M),
                                       n_iter = n_burn + n_save)
  theta_Y <- fit_theta_Y$theta[-(1:n_burn), ] %>% colMeans()
  sigma2_Y <- colMeans(fit_theta_Y$sigma2[-(1:n_burn),])
  ...
  ## Initialize regression from (X, theta_M) to theta_Y ----

  hypers_Y <- Hypers(cbind(X, theta_M), theta_Y)
  forest_Y <- MakeForest(hypers = hypers_Y, opts = opts)

  XQY <- cbind(XQ, pnorm(theta_M))
  mu_theta_Y <- forest_Y$do_gibbs(XQY, theta_Y, XQY, n_burn)
  mu_theta_Y <- forest_Y$do_gibbs(XQY, theta_Y, XQY, n_burn)
  tau2_Y <- forest_Y$get_sigma()^2

  slopes_Y <- update_a_b_sigma(theta_Y, Y, sigma2_Y)
  a_Y <- slopes_Y$a
  b_Y <- slopes_Y$b
  sigma2_Y <- slopes_Y$sigma2
  ...
  inits <- list(theta_Y = theta_Y, theta_M = theta_M, forest_Y = forest_Y,
                forest_M = forest_M, a_Y = a_Y, b_Y = b_Y, a_M = a_M, b_M = b_M,
                sigma2_Y = sigma2_Y, sigma2_M = sigma2_M, tau2_Y = tau2_Y,
...
npgibbs_iter <- function(state) {
  ...
  # The thetas
  theta_Y <- update_theta(mu_theta = mu_theta_Y, Y = Y, a = a_Y, b = b_Y,
                          sigma2 = sigma2_Y, tau2 = tau2_Y)

  theta_M <- update_theta_M(mu_theta_M = mu_theta_M, theta_M = theta_M, M = M,
                            a = a_M, b = b_M, sigma2_M = sigma2_M,
                            tau2_M = tau2_M, theta_Y = theta_Y,
                            forest_Y = forest_Y, XQ = XQ)

  state$theta_Y <- theta_Y
...
run_npgibbs_latent <- function(X, Y, M, treatment_col = 1, n_burn = 2500, n_save = 2500) {
  # Initialize state
  ...
  samples <- list(
    theta_Y = matrix(NA, n_save, n),
    theta_M = matrix(NA, n_save, n),
    a_Y = matrix(NA, n_save, n_y),
    b_Y = matrix(NA, n_save, n_y),
    a_M = matrix(NA, n_save, n_m),
    b_M = matrix(NA, n_save, n_m),
    sigma2_Y = matrix(NA, n_save, n_y),
    sigma2_M = matrix(NA, n_save, n_m),
    tau2_Y = numeric(n_save),
    tau2_M = numeric(n_save),
    # direct_effects = matrix(NA, n_save, n_y),
    # indirect_effects = matrix(NA, n_save, n_y)
    ...
    # NEW: PATE via Bayesian bootstrap
  ...
  # Save samples and compute mediation effects
  for(i in 1:n_save) {
    pb$tick()
    state <- npgibbs_iter(state)

    # Store basic parameters
    samples$theta_Y[i,] <- state$theta_Y
    samples$theta_M[i,] <- state$theta_M
    samples$a_Y[i,] <- state$a_Y
    samples$b_Y[i,] <- state$b_Y
    samples$a_M[i,] <- state$a_M
    samples$b_M[i,] <- state$b_M
    samples$sigma2_Y[i,] <- state$sigma2_Y
    ...
    # Resampled M's
    epsilon_M <- rnorm(n = n, mean = 0, sd = sqrt(state$tau2_M))
    M_1 <- state$forest_M$do_predict(XQ_1) + epsilon_M
    M_0 <- state$forest_M$do_predict(XQ_0) + epsilon_M

    # Resampled Y's
    epsilon_Y <- rnorm(n = n, mean = 0, sd = sqrt(state$tau2_Y))
    ...
    direct_0 <- colMeans(Y_10 - Y_00)   # X->Y holding M at M(0)
    direct_1 <- colMeans(Y_11 - Y_01)   # X->Y holding M at M(1)
    indirect_0 <- colMeans(Y_01 - Y_00)  # X->M->Y holding X at 0
    indirect_1 <- colMeans(Y_11 - Y_10)  # X->M->Y holding X at 1

    samples$direct_0[i] <- direct_0
    samples$direct_1[i] <- direct_1
    samples$indirect_0[i] <- indirect_0
    samples$indirect_1[i] <- indirect_1

    # ---- PATE: Bayesian bootstrap weights  ~ Dir(1,...,1) ----
    w <- rdirichlet_1(rep(1, n))

    direct_0_pate   <- sum(w * (Y_10 - Y_00))
    direct_1_pate   <- sum(w * (Y_11 - Y_01))
    indirect_0_pate <- sum(w * (Y_01 - Y_00))
    indirect_1_pate <- sum(w * (Y_11 - Y_10))

    samples$direct_0_pate[i]   <- direct_0_pate
    samples$direct_1_pate[i]   <- direct_1_pate
    samples$indirect_0_pate[i] <- indirect_0_pate
    samples$indirect_1_pate[i] <- indirect_1_pate
  }
